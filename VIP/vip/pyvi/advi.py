...
class ADVI():
    ...
    def __init__(self, lnprob, kernel='meanfield', nmc=4, mu0=None, omega0=None, out='samples.hdf5'):
        '''
    ...
    def sample(self, optimizer='sgd', n_iter = 1000, stepsize = 1e-3, gamma=1.0, decay_step=1, alpha=0.9,
        ...
        op = optm.optimizer(x.shape, self.grad, method=optimizer, alpha=alpha, beta=beta)
        x, losses = op.optim(x, n_iter=n_iter, stepsize=stepsize, gamma=gamma, decay_step=decay_step)


        # f = h5py.File(self.out,'w')
        # samples = f.create_dataset('samples',(1,x.shape[0]),
