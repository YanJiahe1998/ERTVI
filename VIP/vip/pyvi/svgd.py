...
class SVGD():
    ...
    def grad(self, theta, mkernel=None, chunks=None):
        ...
        if(chunks is None):
            chunks = theta.shape
        ############################################################################################
        # logp, grad, mask, self.isstop, Jacobi_grad = self.lnprob(theta)
        logp, grad, mask, self.isstop = self.lnprob(theta)
        ############################################################################################
        if(mask is None):
        ...
        ############################################################################################
        # return -logp, grad, mask, Jacobi_grad
    ...
    def __dsample(self, theta, n_iter=100, stepsize=1e-2, gamma=1.0, decay_step=1, alpha=0.9, chunks=None):
        ...
        for i in range(n_iter):
            ...
            # loss, grad, mask, Jacobi_grad = self.grad(theta, mkernel=mkernel, chunks=chunks)
...
class sSVGD():
    ...
    def ma_sample(self, x0, samples, n_iter=1000, stepsize=1e-2, gamma=1.0, decay_step=1,
        ...
        for i in range(n_iter):
            ...
            weight_=0.01
            update_step = stepsize*sgrad + stepsize*random_update*weight_
            print('max and min random:',np.max(stepsize*random_update*weight_),np.min(stepsize*random_update*weight_))
            print('max and min update_step:',np.max(update_step),np.min(update_step))
